...
"\n"
"The default usage of the tool is to point it with a single argument\n"
"to a directory in which there is a 'zummary' and a 'benchmarks' file.\n"
"It will then try to produce an optimized order of the benchmarks with\n"
"consecutive benchmarks put into the same bucket to be run on the same\n"
"node, i.e., assigned to the same array job, which is assigned a fixed\n"
"number of cores and a fixed memory budget per job / bucket. Defaults are\n"
"picked for the bwHPC Helix cluster we are currently using.\n"
"\n"
"But in principle the tool is given two arguments: a 'benchmarks' file and a\n"
...
"in the given directory.  If both are giving, i.e., a directory and a file\n"
"they can occur in an arbitrary order. The tool then reads both files and\n"
"tries to match names.  If this is successful it sorts the benchmarks\n"
...
"\n"
"Our primary goal is to maximize memory usage per job / benchmark, while\n"
...
"Ultimately our objective is to minimize the running cost in terms of power\n"
"needed for the number of allocated cores and in turn the wall-clock for\n"
"completion of the whole array job.\n"
